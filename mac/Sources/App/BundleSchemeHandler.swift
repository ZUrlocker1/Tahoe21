...
///
/// 1. `index.html` references `styles.css?v=1.0.0` and `app.js?v=1.0.0`. Those
...
final class BundleSchemeHandler: NSObject, WKURLSchemeHandler {
    ...
    func webView(_ webView: WKWebView, start urlSchemeTask: WKURLSchemeTask) {
        guard let url = urlSchemeTask.request.url, let file = resolve(url) else {
            NSLog("[Tahoe21] scheme handler could not resolve \(urlSchemeTask.request.url?.absoluteString ?? "nil")")
