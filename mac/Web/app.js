...
function showTurnPrompt(defaultText = "Your move.") {
  if (canSplit()) {
    const splitText = isCompactLayout() ? "Split: create 2 hands?" : "Split available: create two hands (adds one matching bet).";
...
function settleInitialBlackjack() {
  ...
  const resolveOutcome = () => {
    if (playerEval.isBlackjack && dealerEval.isBlackjack) {
      state.balance += hand.bet;
      pushSound();
      setResult("Push. Both have blackjack.", `Net ${formatSigned(0)}. Press Next...`, "is-neutral");
      ...
      setResult(
        ...
        `Net ${formatSigned(profit)}. Press Next...`,
        "is-win"
      );
    } else {
      losingHandSound();
      setResult(
        "Dealer blackjack.",
        `You lose ${formatCash(hand.bet)}. Net ${formatSigned(-hand.bet)}. Press Next...`,
        "is-loss"
      );
    }
...
function split() {
  ...
  const revealSecondTimer = setTimeout(() => {
    ...
    state.phase = "PLAYER_TURN";
    showTurnPrompt("Play first hand.");
    renderAll();
...
function finalizeDealerRound(dealerEval) {
  ...
  let sub = `Net ${formatSigned(net)}. Press Next...`;
...
function toggleTestMode() {
  ...
  if (state.testMode) {
    ...
    state.testRoundDeck = null;
    state.testRoundPos = 0;
    const compactOffText = state.phase === "ROUND_RESULT" ? "Press Next..." : "Press Deal to start.";
