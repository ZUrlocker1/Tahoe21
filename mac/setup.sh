#!/bin/bash
# setup.sh — regenerate Tahoe21.xcodeproj from project.yml.
#
#   bash setup.sh
#
...
if [[ ! -f "Web/index.html" ]]; then
    fail "Web/index.html missing. Copy it from the Tahoe21 web repo:"
    fail "  cp -R ../Tahoe21/app/{index.html,styles.css,app.js,assets} Web/"
    exit 1
fi
...
# ── 5. Generate ──────────────────────────────────────────────────────────────
step "Generating Tahoe21.xcodeproj"
xcodegen generate
...
echo ""
echo -e "  1. Open it:"
echo -e "     ${CYAN}open Tahoe21.xcodeproj${NC}"
echo ""
